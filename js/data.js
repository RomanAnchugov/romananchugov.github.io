...
const TEXT = {
  ...
  meta: {
    title: "RomanAn — мобильная разработка под ключ | iOS, Android, KMP",
    description: "Приложения под ключ для iOS и Android, ТЗ для бизнеса, менторинг, AI-assisted разработка."
  },
  ...
  hero: {
    ...
    sub: "Роман Анчугов — мобильный разработчик полного цикла. Проектирую, пишу код и довожу продукт до релиза на iOS и Android, ускоряя часть этапов нейросетями, но не в ущерб качеству.",
    cta: [
  ...
  sections: {
    ...
    experience: { title: "Опыт и проекты", note: "", sub: "Весь коммерческий опыт подтверждается записями в трудовой книжке. Никаких накруток и вранья." }
  },
  ...
  contactTitle: "Есть идея приложения? Пишите по любому поводу — это бесплатно. Обсудим, что нужно, и во сколько это встанет.",
...
const WHY = [
  { title: "Опыт в биг-техе", text: "Работал в крупных продуктовых компаниях с высокими стандартами кода, ревью и релизных процессов." },
  { title: "Многогранный опыт", text: "Fintech, Foodtech, Funtech, e-commerce — разные домены и разные классы задач под iOS и Android." },
...
const JOBS = [
  {
    ...
    screens: [
      { placeholder: "Здесь может быть ваше приложение" }
    ]
  },
  {
    period: "2023 — 2025", place: "Авито", role: "Senior Mobile Developer",
    summary: "Разрабатывал видео-платформу на Авито с нуля — видео-лента, видео-сниппеты, видео в объявлениях. Повысил производительность видео в приложении в 3 раза (подтверждено техническими АБ-тестами) — разработал компонент-оркестратор с большим количеством низкоуровневых оптимизаций. Являлся фича-лидом рассрочек (BNPL) в краткосрочной аренде жилья.",
    stack: ["Kotlin", "Google Maps SDK", "WorkManager"],
    screens: [
      { caption: "Поиск жилья на карте", src: "img/avito1.webp", pos: "center" },
      { caption: "Посуточная аренда", src: "img/avito2.webp", pos: "center" }
    ]
  },
  {
  ...
  },
  {
    ...
    summary: "Разрабатывал Тинькофф Инвестиции. Был в Base команде. Занимался основными экранами — портфель, покупка/продажа, стакан и т.д. Высоконагруженные экраны: всё постоянно меняется каждую секунду и должны быть отказоустойчивыми(т.к. тут деньги). Также разрабатывал сложную фичу по авто-инвестированию.",
